...
from catalog.models import Field, ET0Daily, Crop, SoilMoistureMeasurement
import requests
...
def calculate_irrigation_for_field(field_id, crop_override_id=None):
    try:
        ...
        return {"error": "Neznáme pole"}

    day_of_season = get_day_of_season(field.sowing_date)

    crop_id = crop_override_id

    if crop_id:
        try:
            crop = Crop.objects.get(id=crop_id)
        except Crop.DoesNotExist:
            return {"error": "Neznáma plodina"}
    else:
        if not field.crop_id:
            return {"error": "Pole nemá priradenú plodinu"}
        crop = field.crop

    if not field.soil_type_id:
        return {"error": "Pole nemá priradený typ pôdy"}

    soil = field.soil_type

    row = ET0Daily.objects.filter(field_id=field_id).order_by("-date").first()
    if not row:
        return {"error": "Nemám ET0 dáta pre toto pole"}

    et0 = Decimal(str(row.et0_mm))
    kc = get_kc_for_day(crop, day_of_season)
    etc = et0 * kc

    zr = field.root_depth_effective()
    p = field.p_effective()

    if zr is None:
        return {"error": "Nie je určená hĺbka zakorenenia"}
    ...
    return {
        ...
        "sensor_deficit_mm": round(float(dr_sensor), 2) if dr_sensor is not None else None,
