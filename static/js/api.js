...
// ------chart js -----//
async function loadET0() {
    const response = await fetch("/api/fields/1/et0/");
    const data = await response.json();

    return data.map(item => item.et0);
}
...
async function loadFieldName() {
    const response = await fetch("/api/fields/");
    const fields = await response.json();

    console.log("fields z API:", fields);

    if (fields.length > 0) {
        document.getElementById("field-name").innerText = fields[0].name;
    }
}
