...
document.getElementById("crop-select").addEventListener("change", async function () {
    ...
    try {
        ...
        document.getElementById("kc-value").innerText =
            data.crop?.kc ?? "--";

        document.getElementById("etc-value").innerText =
            data.etc_mm != null ? `${data.etc_mm} mm` : "--";

        document.getElementById("irrigation-status").innerText =
            data.zavlazovat ? "Zavlažovať" : "Nezavlažovať";

        document.getElementById("irrigation-dose-card").innerText =
            data.doporucana_davka_mm != null ? data.doporucana_davka_mm.toFixed(2) : "0.00";

    } catch (error) {
        console.error("Chyba pri výpočte plodiny:", error);
    }
...
async function loadEt0History(fieldId) {
    console.log("loadEt0History fieldId:", fieldId);
    const chartContainer = document.getElementById("et0-history-chart");
    const listContainer = document.getElementById("et0-history-list");

    if (!chartContainer || !fieldId) return;
    ...
    try {
        ...
        const values = (data.data || []).map(item => Number(item.et0_mm));
        ...
        et0Chart = new ApexCharts(chartContainer, {
            chart: {
                type: "line",
                height: 320,
                toolbar: {
        ...
        chartContainer.innerHTML = `<div class="text-danger">Nepodarilo sa načítať ET0 graf.</div>`;
        console.error("Chyba ET0 grafu:", error);
    }
}

async function loadLatestSoilMoisture(fieldId) {
    ...
    try {
        ...
        if (!data.has_data) {
            box.innerHTML = `
                <div class="text-muted">Pre toto pole zatiaľ nie sú dostupné dáta zo senzora.</div>
            `;
            return;
        }
            ...
            <div class="row g-3">
                <div class="col-md-3">
                    <div class="border rounded p-3 h-100">
                        <div class="text-muted">Senzor</div>
                        <div class="fs-5 fw-bold">${data.sensor ?? "--"}</div>
                    </div>
                </div>

                <div class="col-md-3">
                    <div class="border rounded p-3 h-100">
                        <div class="text-muted">Vlhkosť pôdy</div>
                        <div class="fs-4 fw-bold">${data.vwc_percent != null ? data.vwc_percent.toFixed(1) + " %" : "--"}</div>
                    </div>
                </div>

                <div class="col-md-3">
                    <div class="border rounded p-3 h-100">
                        <div class="text-muted">Teplota</div>
                        <div class="fs-4 fw-bold">${data.temperature_c != null ? data.temperature_c.toFixed(1) + " °C" : "Nedostupné"}</div>
                    </div>
                </div>

                <div class="col-md-3">
                    <div class="border rounded p-3 h-100">
                        <div class="text-muted">Čas merania</div>
                        <div class="fw-bold">${data.measured_at ? new Date(data.measured_at).toLocaleString("sk-SK") : "--"}</div>
                    </div>
                </div>
            </div>
        `;
    } catch (error) {
        box.innerHTML = `<div class="text-danger">Nepodarilo sa načítať dáta zo senzora.</div>`;
        console.error("Chyba senzora:", error);
    }
}
